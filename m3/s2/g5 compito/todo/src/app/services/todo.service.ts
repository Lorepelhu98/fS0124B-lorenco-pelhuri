  ...
  completed: any;
    ...
    assignUsersToTodos(): void {
      this.todos.forEach(todo => {
        const user = this.userSrv.getUserById(todo.userId);
        if (user) {
          todo.user = user;
        }
      });
    ...
    getUsersWithTheirTodos(): User[] {
      return this.userSrv.getAllUsers().map(user => {
        user.todos = this.todos.filter(todo => todo.userId === user.id);
        return user;
      });
    }

    filterByQuery(query: string): Todo[] {
      return this.todos.filter(todo => todo.todo.toLowerCase().includes(query.toLowerCase()));
    }

    mapByUser(): Observable<User[]> {
      return of(this.userSrv.getAllUsers());
    }
  }
